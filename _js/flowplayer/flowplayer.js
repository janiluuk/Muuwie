...
$(function() {
   if (typeof jQuery.fn.flowplayer == 'function') {
...
window.flowplayer = function(fn) {
   return use_native ? 0 :
      jQuery.isFunction(fn) ? extensions.push(fn) :
      typeof fn == 'number' || fn === undefined ? instances[fn || 0] :
      $(fn).data("flowplayer");
};


jQuery.extend(flowplayer, {
...
// jQuery plugin
jQuery.fn.flowplayer = function(opts, callback) {

   if (typeof opts == 'string') opts = { swf: opts }
   if (jQuery.isFunction(opts)) { callback = opts; opts = {} }

   return !opts && this.data("flowplayer") || this.each(function() {

      // private variables
      var root = $(this).addClass("is-loading"),
         conf = jQuery.extend({}, flowplayer.defaults, flowplayer.conf, opts, root.data()),
      ...
      var api = {
         ...
         load: function(video, callback) {
            ...
            jQuery.extend(video, engine.pick(video.sources));
            ...
            if (video.src) {
               ...
               if (!e.isDefaultPrevented()) {
                  ...
                  // callback
                  if (jQuery.isFunction(video)) callback = video;
         ...
         */
         seek: function(time, callback) {
            if (api.ready) {
               ...
               engine.seek(time);
               if (jQuery.isFunction(callback)) root.one("seek", callback);
            }
            return api;
         },
         ...
         speed: function(val, callback) {
            ...
            if (api.ready) {
               ...
               if (typeof val == "boolean") {
                  val = conf.speeds[jQuery.inArray(api.currentSpeed, conf.speeds) + (val ? 1 : -1)] || api.currentSpeed;
               }
      ...
      jQuery.each(['bind', 'one', 'unbind'], function(i, key) {
      ...
      root.bind("boot", function() {
         ...
         // conf
         jQuery.each(['autoplay', 'loop', 'preload', 'poster'], function(i, key) {
         ...
         // 2. failed -> try another
            ...
            jQuery.each(flowplayer.engine, function(name, impl) {
               if (name != conf.engine) {
...
function embed(swf, flashvars) {
   ...
   tag += jQuery.browser.msie ? 'classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">' :
   ...
   var opts = {
      ...
      movie: swf + (jQuery.browser.msie ? "?" + id : ""),
      name: id
   };

   // flashvars
   jQuery.each(flashvars, function(key, value) {
...
flowplayer.engine.flash = function(player, root) {
   ...
   var engine = {
      ...
      pick: function(sources) {
         ...
         if (flowplayer.support.flashVideo) {
            ...
            var flash = jQuery.grep(sources, function(source) { return source.type == 'flash'; })[0];
      ...
      load: function(video) {
         ...
         if (api) {
            ...
            // optional conf
            jQuery.each(['key', 'autoplay', 'preload', 'rtmp', 'loop', 'debug'], function(i, key) {
            ...
            $[callbackId] = function(type, arg) {
               ...
               switch (type) {
                  ...
                  case "ready": arg = jQuery.extend(video, arg); break;
   ...
   jQuery.each("pause,resume,seek,volume".split(","), function(i, name) {
...
flowplayer.engine.html5 = function(player, root) {
   ...
   function listen(api, sources, video) {
      ...
      jQuery.each(EVENTS, function(type, flow) {
         ...
         api.addEventListener(type, function(e) {
            ...
            var event = jQuery.Event(flow), arg;

            switch (flow) {

               case "ready":

                  arg = jQuery.extend(video, {
...
function URLResolver(videoTag) {
   ...
   self.resolve = function(video) {
      ...
      if (jQuery.isArray(video)) {

         video = { sources: jQuery.map(video, function(el) {
            var type; jQuery.each(el, function(key, value) { type = key; });
            el.type = type;
            el.src = el[type];
            delete el[type];
            return el;
   ...
   return this.each(function() {
      ...
      root.data("api", api).bind("mousedown.sld", function(e) {
         ...
         if (!disabled) {
            ...
            var delayedFire = jQuery.throttle(fire, 100);
            calc();
            api.dragging = true;
            fire(mousemove(e));
...
flowplayer(function(api, root) {
   ...
   /* controlbar elements */
   jQuery.each(['mute', 'fullscreen', 'unload'], function(i, key) {
...
$(document).bind("keydown.fp", function(e) {
   ...
   if (jQuery.inArray(key, [63, 187, 191, 219]) != -1) {
...
var VENDOR = jQuery.browser.mozilla ? "moz": "webkit",
   FS_ENTER = "fullscreen",
...
flowplayer(function(player, root) {
   ...
   var conf = jQuery.extend({ active: 'is-active', advance: true, query: ".fp-playlist a" }, player.conf),
      klass = conf.active;
   ...
   if (els().length) {
      ...
      // api.next() / api.prev()
      jQuery.each(['next', 'prev'], function(i, key) {
...
flowplayer(function(player, root) {
   ...
   if (player.conf.generate_cuepoints) {
      ...
      player.bind("ready", function() {
         ...
         jQuery.each(cues, function(i, cue) {
...
flowplayer(function(player, root, engine) {
   ...
   jQuery.get(url, function(txt) {
      ...
      for (var i = 0, lines = txt.split("\n"), len = lines.length, entry = {}, title, timecode, text, cue; i < len; i++) {
         ...
         if (timecode) {
            ...
            // text
            text = "<p>" + lines[++i] + "</p><br/>";
            while (jQuery.trim(lines[++i]) && i < lines.length) text +=  "<p>" + lines[i] + "</p><br/>";
   ...
   player.bind("cuepoint", function(e, api, cue) {
      ...
      jQuery.each(player.cuepoints || [], function(i, cue) {
...
flowplayer(function(player, root) {
   ...
   if (id) {
      ...
      if (typeof _gat == 'undefined') jQuery.getScript("http://www.google-analytics.com/ga.js");
...
if (flowplayer.support.touch) {
   ...
   flowplayer(function(player, root) {
      ...
      // native fullscreen
      if (player.conf.native_fullscreen && jQuery.browser.webkit) {
...
flowplayer(function(player, root) {
   ...
   player.embedCode = function() {
      ...
      // configuration
      jQuery.each(['origin', 'analytics', 'logo', 'key', 'rtmp'], function(i, key) {
         if (conf[key]) el.attr("data-" + key, conf[key]);
      });

      // sources
      jQuery.each(video.sources, function(i, src) {
...
jQuery.fn.fptip = function(trigger, active) {
